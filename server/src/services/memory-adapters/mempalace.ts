...
export function createMempalaceMemoryAdapter(
  ...
  async function write(req: MemoryWriteRequest): Promise<{
    ...
    if (req.mode === "upsert" && req.metadata?.drawerId) {
      ...
      const realDrawerId = parsed?.drawer_id as string | undefined;
      const newDrawerId = realDrawerId ?? `drawer-${Date.now()}`;
      if (!realDrawerId) {
        console.warn("[mempalace] upsert: mempalace_add_drawer did not return drawer_id; skipping old drawer delete to prevent data loss", text);
      }

      // Only delete old drawer when mempalace confirmed the new one with a real id.
      // Without confirmation, keep both — duplicates are safer than data loss.
      if (realDrawerId) {
        try {
  ...
  async function get(
    ...
    // mempalace does not expose a get_drawer tool, so we approximate by
    // searching with wing/room filters and the drawerId as query text.
    // This is a best-effort lookup — callers should not depend on get()
    // for correctness when exact-match retrieval is required.
    const toolArgs: Record<string, unknown> = {
      query: drawerId,
      limit: 1,
    };
