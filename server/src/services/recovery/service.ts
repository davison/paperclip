...
export function recoveryService(db: Db, deps: { enqueueWakeup: RecoveryWakeup }) {
      ...
      const postedComment = await db
        ...
        .where(
          and(
            ...
            eq(issueComments.createdByRunId, candidate.id),
          ),
        )
        .limit(1)
        .then((rows) => rows[0] ?? null);
  ...
  async function reconcileStrandedAssignedIssues() {
    ...
    for (const issue of candidates) {
      ...
      // Empty-response circuit breaker (RED-160). The continuation-recovery
      // escalation above only fires when the previous reconcile re-fire was the
      // latest run; intermediate `missing_issue_comment` retries (queued by
      ...
      // an unbounded heartbeat budget on the same issue.
