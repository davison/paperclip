...
import {
  resolvePluginManifestEntrypoint,
...
describe("resolvePluginWatchTargets", () => {
  ...
  it("resolvePluginManifestEntrypoint returns the absolute manifest path declared in package.json", () => {
    const pluginDir = makeTempPluginDir();
    ...
    expect(resolvePluginManifestEntrypoint(pluginDir)).toBe(
      path.join(pluginDir, "dist", "manifest.js"),
    );
  });

  it("resolvePluginManifestEntrypoint returns null when package.json does not declare a manifest entrypoint", () => {
    const pluginDir = makeTempPluginDir();
    writeFileSync(
      path.join(pluginDir, "package.json"),
    ...
    expect(resolvePluginManifestEntrypoint(pluginDir)).toBeNull();
  });

  it("resolvePluginManifestEntrypoint returns null when package.json is missing", () => {
    const pluginDir = makeTempPluginDir();
    expect(resolvePluginManifestEntrypoint(pluginDir)).toBeNull();
  });
