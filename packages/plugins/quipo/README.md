...
> **Status: release candidate (`0.1.0-rc.1`).** Always pin the `@rc` dist-tag while the package is in release-candidate state — `latest` is reserved for the eventual `0.1.0` final.

1. **Install Quipo into your Paperclip instance.**

   Install from the npm registry with the `@rc` dist-tag:
   ...
   > **Maintainers note:** publish only via `pnpm publish` from the monorepo root. `npm pack` / `npm publish` do not rewrite `workspace:*` ranges, so the resulting tarball will fail to install on consumers.
